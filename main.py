...
class EmbeddingCache:
    ...
    def recognize(self, embedding: np.ndarray, threshold: Optional[float] = None) -> Optional[Dict[str, Any]]:
        ...
        if norm < 1e-8:
            logger.warning("Recognition: Probe embedding norm is near zero.")
            return None
        ...
        # Logging audit: best_match, similarity_score, threshold, match/reject reason
        if best_entry is not None:
            logger.info(
                "Recognition match attempt: best_match=%s (%s), similarity_score=%.4f, threshold=%.4f",
                best_entry.user_id, best_entry.name, best_score, thr
            )
        else:
            logger.info("Recognition match attempt: no enrolled users found.")

        if best_entry is None or best_score < thr:
            reason = "no enrolled users in cache" if best_entry is None else f"similarity_score {best_score:.4f} below threshold {thr:.4f}"
            logger.info("Recognition: Rejected. Reason: %s", reason)
            return None

        logger.info(
            "Recognition: Matched! subject_id=%s, name=%s, employee_code=%s, score=%.4f, threshold=%.4f",
            best_entry.user_id, best_entry.name, best_entry.employee_code, best_score, thr
        )
...
class RecognitionPipeline:
    ...
    def detect_and_validate(self, frame: np.ndarray) -> Optional[FaceDetection]:
        ...
        # Logging audit: face_detected
        if detection is None:
            self._log.debug("Recognition: No face detected in frame.")
            return None

        self._log.info("Recognition: Face detected with confidence %.3f", detection.confidence)

        if not detection.is_valid:
            self._log.info(
                "Recognition: Face rejected by detector checks: %s (conf=%.3f, blur=%.1f, brightness=%.1f)",
        ...
        if detection.confidence < self._config.min_face_confidence:
            self._log.info("Recognition: Face rejected — confidence (%.3f) < min_face_confidence (%.3f)", 
                           detection.confidence, self._config.min_face_confidence)
            return None

        if detection.blur_score < self._config.min_blur_threshold:
            self._log.info("Recognition: Face rejected — blur (%.1f) < min_blur_threshold (%.1f)", 
        ...
        if not (
            ...
            self._log.info("Recognition: Face rejected — brightness (%.1f) out of range [%.1f, %.1f]", 
                           detection.brightness, self._config.min_brightness, self._config.max_brightness)
            return None
    ...
    def extract_embedding(self, detection: FaceDetection) -> Optional[np.ndarray]:
        try:
            ...
            # Logging audit: embedding_generated
            if embedding is None:
                self._log.warning("Recognition: Embedding extraction returned None.")
                return None
            if embedding.ndim != 1 or embedding.shape[0] == 0:
                self._log.warning("Recognition: Unexpected embedding shape: %s", embedding.shape)
                return None
                
            self._log.info("Recognition: Embedding generated successfully (dim=%d).", embedding.shape[0])
            return embedding
        except Exception as exc:
            self._log.error("Recognition: Embedding extraction failed: %s", exc, exc_info=True)
            return None
...
class CameraProcessor:
    ...
    def _draw_overlay(
        ...
        """Annotate frame with recognition result, showing name if present. Does not call imshow."""
        ...
        # Build label with name if available
    ...
    def _draw_hud(self, frame: np.ndarray) -> None:
        ...
        # ── live frame diagnostics ──────────────────────────────────────
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        brightness = float(np.mean(gray))
        blur_score = float(cv2.Laplacian(gray, cv2.CV_64F).var())

        # Colour-code brightness warning
        if brightness > self._config.max_brightness:
            bright_color = (0, 60, 255)   # red — too bright
            bright_label = f"Brightness: {brightness:.0f}  TOO BRIGHT (max {self._config.max_brightness:.0f})"
        elif brightness < self._config.min_brightness:
            bright_color = (0, 140, 255)  # orange — too dark
            bright_label = f"Brightness: {brightness:.0f}  TOO DARK (min {self._config.min_brightness:.0f})"
        else:
            bright_color = (80, 220, 80)  # green — OK
            bright_label = f"Brightness: {brightness:.0f}  OK"

        blur_ok = blur_score >= self._config.min_blur_threshold
        blur_color = (80, 220, 80) if blur_ok else (0, 60, 255)
        blur_label = f"Blur: {blur_score:.0f}  {'OK' if blur_ok else f'TOO BLURRY (min {self._config.min_blur_threshold:.0f})'}"

        # Semi-transparent top diagnostic bar
        top_h = 52
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, 0), (w, top_h), (0, 0, 0), -1)
        cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)

        cv2.putText(frame, bright_label, (10, 18),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.52, bright_color, 1)
        cv2.putText(frame, blur_label, (10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.52, blur_color, 1)

        # Threshold reminder top-right
        thr_text = f"Threshold: {self._config.recognition_threshold:.2f}"
        ...
        banner_h = 36
        overlay2 = frame.copy()
        cv2.rectangle(overlay2, (0, h - banner_h), (w, h), (0, 0, 0), -1)
        cv2.addWeighted(overlay2, 0.55, frame, 0.45, 0, frame)

        left_text = f"Enrolled: {enrolled}"
        right_text = "[Q] Return to Menu"

        cv2.putText(frame, left_text, (10, h - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (200, 200, 200), 1)
        (txt_w, _), _ = cv2.getTextSize(right_text, cv2.FONT_HERSHEY_SIMPLEX, 0.55, 1)
        cv2.putText(frame, right_text, (w - txt_w - 10, h - 10),
    ...
    def _handle_keypress(self, key: int) -> None:
        ...
        Q (113) / ESC (27) — return to the main menu.
        """
        if key == ord("q") or key == 27:
            self._log.info("Q / ESC pressed — returning to main menu")
            self._shutdown.set()
...
class DrishtiApplication:
    ...
    def _init_models(self) -> None:
        ...
        # Create independent face detector instances for enrollment and recognition
        # to avoid thread-safety issues and MediaPipe tracking state pollution.
        # Pass configured thresholds from SystemConfig.
        self._enrollment_detector = FaceDetector(
            blur_threshold=self._config.min_blur_threshold,
            brightness_min=self._config.min_brightness,
            brightness_max=self._config.max_brightness,
            min_detection_confidence=self._config.min_face_confidence,
        )
        self._recognition_detector = FaceDetector(
            blur_threshold=self._config.min_blur_threshold,
            brightness_min=self._config.min_brightness,
            brightness_max=self._config.max_brightness,
            min_detection_confidence=self._config.min_face_confidence,
        )
        self._log.info("Independent FaceDetectors created with configured thresholds.")
    ...
    def _init_recognition(self) -> None:
        ...
        self._enrollment_manager = EnrollmentManager(
            db_manager=self._db_manager,
            face_detector=self._enrollment_detector,
            model=self._model,
        )
    ...
    # ------------------------------------------------------------------
    # Public helpers called by the main menu
    # ------------------------------------------------------------------

    def get_enrolled_count(self) -> int:
        """Return number of users currently in the embedding cache."""
        if self._embedding_cache is None:
            return 0
        return self._embedding_cache.get_enrolled_count()

    def run_recognition(self) -> int:
        """
        Open the camera and run the recognition loop until the user presses
        Q / ESC.  Returns to the main menu when the loop exits.
        A fresh per-session shutdown event is used so recognition can be
        started and stopped multiple times without restarting the process.
        """
        session_shutdown = threading.Event()

        recognition_pipeline = RecognitionPipeline(
            face_detector=self._recognition_detector,
            model=self._model,
        ...
        # Start (or reuse) the background sync worker once per process.
        if not hasattr(self, "_sync_worker") or self._sync_worker is None or not self._sync_worker.is_alive():
        ...
        camera_processor = CameraProcessor(
            ...
            embedding_cache=self._embedding_cache,
            shutdown_event=session_shutdown,
        )

        self._log.info("=" * 60)
        self._log.info("NHAI Drishti — RECOGNITION MODE")
        self._log.info("Press Q or ESC in the camera window to return to menu.")
        self._log.info("=" * 60)
        ...
        try:
            ...
            self._log.critical(
                "Fatal error in recognition camera loop: %s", exc, exc_info=True
            )
            return 1

        # Reload embeddings after returning from recognition (in case
        # the operator ran enrollment in a previous session).
        self._embedding_cache.load_embeddings()
        return 0

    def run_enrollment_interactive(self) -> None:
        """
        Open the camera, collect employee details on stdin, capture face
        frames, and store the embedding.  Fully self-contained — no
        secondary thread or shutdown event needed.
        """
        import cv2 as _cv2

        self._log.info("Opening camera for enrollment …")
        cap = _cv2.VideoCapture(self._config.camera_index)
        if not cap.isOpened():
            print(
                f"\n  [ERROR] Cannot open camera at index {self._config.camera_index}.\n"
                "  Check that no other application is using the camera.\n"
            )
            self._log.error(
                "Enrollment aborted — camera index=%d unavailable.",
                self._config.camera_index,
            )
            return

        cap.set(_cv2.CAP_PROP_FRAME_WIDTH, self._config.camera_width)
        cap.set(_cv2.CAP_PROP_FRAME_HEIGHT, self._config.camera_height)
        cap.set(_cv2.CAP_PROP_FPS, self._config.camera_fps)
        ...
        try:
            ...
            print("  Capturing frames for quality analysis.\n")
            ...
            if report.success:
                ...
                print(f"     Frames passed QC     : {report.frames_passed_quality}")
                self._log.warning(
...
def _print_banner() -> None:
    print("\n" + "=" * 60)
    print("  NHAI Drishti — Offline Face Recognition Attendance System")
    print("=" * 60)


def _print_menu(enrolled_count: int) -> str:
    """
    Display the main mode-selection menu and return the validated choice.
    Loops until the user enters a valid option.
    """
    while True:
        print(f"\n  Enrolled users : {enrolled_count}")
        print("\n  Select mode:")
...
def main() -> int:
    ...
    # -----------------------------------------------------------------------
    # Mode-selection loop — user explicitly picks Enrollment or Recognition
    # each time they return to the menu.
    # -----------------------------------------------------------------------
    while True:
        enrolled_count = app.get_enrolled_count()
        choice = _print_menu(enrolled_count)
        ...
        if choice in ("q", "quit", "exit"):
            ...
            if enrolled_count == 0:
                print(
                    "\n  [WARNING] No users enrolled yet.\n"
                    "  Recognition will run but nobody can be identified.\n"
                    "  Enroll at least one user first (choose option 2).\n"
                )
            ...
            print("  Press  Q  or  ESC  in the camera window to return to menu.\n")
            logger.info("Entering RECOGNITION mode.")
            exit_code = app.run_recognition()
            if exit_code != 0:
                logger.error("Recognition mode exited with code %d", exit_code)
